...
export default class FilterComponent extends Component {
  ...
  autoGenerateRuleLogic(filterRules, ruleLogic) {
    ...
    if (ruleLogic !== '') {
      ...
      if (
        !this.getMentionedFilterRuleNumbers(ruleLogic).includes(
          filterRules.length.toString(),
        )
      ) {
   ...
   */
  updateRuleComponents(filterRules, ruleLogic) {
    // Updates Filter component
    ...
    if (this.props.ruleComponents.filterRules.length > 0) {
      ...
      if (
        ...
        exposedInputStreamFields = this.props.getFieldNamesAndTypes(
          this.props.selectedInputRuleTemplate.templates[0]
        ...
        <div style={{ width: '100%', overflowX: 'auto' }}>
          <div style={{ width: '100%', minWidth: 560 }}>
            ...
            <div style={{ float: 'left', width: '30%', height: 30 }}>
              <Typography type="caption">
                <FormattedMessage
                  id="filter.valueAttribute"
                  defaultMessage="Value/Attribute"
                />
              </Typography>
            </div>
            <div style={{ float: 'left', width: '10%', height: 30 }}>
            ...
            {this.props.ruleComponents.filterRules.map((filterRule, index) => (
              <FilterRule
                ...
                elements={this.props.getFieldNames(
                  this.props.selectedInputRuleTemplate.templates[0]
                    .exposedStreamDefinition,
                )}
  ...
  displayAddFilterButton() {
    if (
      this.props.mode !== BusinessRulesConstants.BUSINESS_RULE_FORM_MODE_VIEW
    ) {
      return (
        <IconButton
          color="primary"
    ...
    if (this.props.ruleComponents.filterRules.length > 0) {
      return (
        <TextField
          ...
          error={
            !BusinessRulesUtilityFunctions.isEmpty(this.props.errorStates)
              ? this.props.errorStates.ruleLogic
              : false
          }
          disabled={
            this.props.mode
  ...
  displayContent() {
    if (
      this.props.mode === BusinessRulesConstants.BUSINESS_RULE_FORM_MODE_VIEW
      && this.props.ruleComponents.filterRules.length === 0
      && this.props.ruleComponents.ruleLogic === ''
    ) {
      return (
        <div>
          ...
          <center>
            <Typography type="subheading">
              <FormattedMessage
                id="filter.nofilters"
                defaultMessage="No filters given"
              />
            </Typography>
          </center>
          <br />
